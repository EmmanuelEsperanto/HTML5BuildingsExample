...
const canvasWindowedScaleMode_TEMPLATE = '1';
const canvasWindowedScaleMode = parseInt(canvasWindowedScaleMode_TEMPLATE);
...
var Module = {
	...
	UE4_indexedDBName: 'UE4_assetDatabase_ExpoTown_WebGL', // this should be an ascii ID string without special characters that is unique to the project that is being packaged
	UE4_indexedDBVersion: 202502090704, // Bump this number to invalidate existing IDB storages in browsers.
};
...
Module.arguments = ['../../../ExpoTown_WebGL/ExpoTown_WebGL.uproject','-stdout',];
...
document.addEventListener('touchstart', function(event) {
    ...
    Module.canvas.dispatchEvent(new MouseEvent('mousedown', { 
        clientX: event.touches[0].clientX, 
        clientY: event.touches[0].clientY 
    }));
}, { passive: false }); // Устанавливаем passive: false для предотвращения поведения


let lastDistance = null; // Переменная для хранения предыдущей дистанции между пальцами

document.addEventListener('touchmove', function(event) {
	event.preventDefault();
    if (event.touches.length === 2) { // Если используется два пальца
        ...
        if (lastDistance !== null) {
            ...
            // Создаем событие колесика мыши
            const wheelEvent = new WheelEvent('wheel', {
                deltaY: -delta, // Инвертируем знак для соответствия стандарту
                clientX: (event.touches[0].clientX + event.touches[1].clientX) / 2, // Центр pinch-жеста
                clientY: (event.touches[0].clientY + event.touches[1].clientY) / 2
            });

            // Отправляем событие на элемент canvas
            Module.canvas.dispatchEvent(wheelEvent);
        }
        ...
        lastDistance = distance;
		    ...
		    Module.canvas.dispatchEvent(new MouseEvent('mousemove', { 
        clientX: event.touches[0].clientX, 
        clientY: event.touches[0].clientY 
    }));
	}
...
document.addEventListener('touchend', function(event) {
		//event.preventDefault();
	    if (event.touches.length < 2) {
        lastDistance = null; // Сбрасываем расстояние, когда жест pinch-to-zoom завершен
    }
   // event.preventDefault(); // Предотвращает стандартное поведение браузера
    Module.canvas.dispatchEvent(new MouseEvent('mouseup', { 
...
window.addEventListener('orientationchange', toggleImage);
...
const canvasWindowedUseHighDpi = true;

// Buy definitely disable High DPI on mobile devices
let IsMobileByUserAgentAndroid = navigator.userAgent.match(/Android/i);
let IsMobileByUserAgentIOS = navigator.userAgent.match(/iPhone/i) || navigator.userAgent.match(/iPad/i);
let IsMobileByMediaQuery = window.matchMedia("(pointer:coarse)").matches;
let IsMobile = IsMobileByUserAgentAndroid || IsMobileByUserAgentIOS || IsMobileByMediaQuery;
...
const canvasAspectRatioWidth = 1366;
const canvasAspectRatioHeight = 768;

// Aspect ratio constraints for the canvas. These are used when canvasWindowedScaleMode == 2 (ASPECT).
let miminumAspectRatio = 1.0;
// https://en.wikipedia.org/wiki/Ultrawide_formats
const maximumAspectRatio = 12.0 / 5.0;

// Check if Telegram version is 8.0 or higher
function isValidTelegramVersion() {
    if (!IsTelegram) {
        return false;
    }
...
// Check for vertical view in Telegram builds
function checkTelegramView() {
    if (IsTelegram) {
        let cssWidth = window.innerWidth;
        let cssHeight = window.innerHeight;

        if (cssWidth / cssHeight < 1) {
            $('#verticaltg').fadeIn();
        } else {
            $('#verticaltg').fadeOut();
        }

        if (!isValidTelegramVersion()) {
            $('#button_fullscreen').fadeOut();
        }

        $('#logo_bot_url').removeAttr("href");
    }
}

// Allow Telegram to use any aspect ratio
if (window.Telegram) {
    let telegramApp = window.Telegram.WebApp;
    if (telegramApp && telegramApp.platform !== 'unknown') {
        IsTelegram = true;

        if (telegramApp.platform === "tdesktop" &&
            navigator && navigator.appVersion && navigator.appVersion.includes("Win64")) {
            IsSupportedPlatform = true;
        }

        // why to smoke the wasp? because.
        telegramApp.onEvent('fullscreenFailed', onFullscreenFailed);
        telegramApp.onEvent('invoiceClosed', onInvoiceClosed);

        // enable closing confirmation dialog by default
        telegramApp.enableClosingConfirmation();

        miminumAspectRatio = 0.1;
        console.log('Telegram platform: ' + window.Telegram.WebApp.platform);
        Module.arguments.push("'-LaunchPlatform=Telegram'");

        // check is that android or ios
        if (telegramApp.platform === 'android' || telegramApp.platform === 'ios') {
            IsMobile = true;
        }

        window.addEventListener('resize', checkTelegramView);
        window.addEventListener('orientationchange', checkTelegramView);
    }
}
...
// Configuration variables, feel free to play around with these to tweak.
    const minimumCanvasHeightCssPixels = 48; // the visible size of the canvas should always be at least this high (in CSS pixels)
    let minimumCanvasHeightFractionOfBrowserWindowHeight = 0.9; // and also vertically take up this much % of the total browser client area height.

    // @dawg for debug builds make size a bit smaller
    const cpuprofiler = document.getElementById('cpuprofiler');
    if (cpuprofiler) {
        minimumCanvasHeightFractionOfBrowserWindowHeight = 0.7;
    }
    ...
    // Initialize default values for the WebGL render target size
    let newRenderTargetWidth = canvasAspectRatioWidth;
    let newRenderTargetHeight = canvasAspectRatioHeight;

    let cssWidth = window.innerWidth; //document.body.clientWidth;
    let cssHeight = Math.max(minimumCanvasHeightCssPixels, window.innerHeight);

    /* NONE */
    if (canvasWindowedScaleMode === 3) {
        // In fixed display mode, render to a statically determined WebGL render target size.
        // So, change nothing
    } else {
        // @dawg Force use full height if available
        minimumCanvasHeightFractionOfBrowserWindowHeight = 1.0;
        const cssAspectRatio = cssWidth / cssHeight;
        let desiredAspectRatio = canvasAspectRatioWidth / canvasAspectRatioHeight;

        const epsilon = 0.01;

        // Limit minimum aspect ratio with 4:3 and maximum aspect ratio with UW max
        if (canvasWindowedScaleMode === 1 /* STRETCH */) {
            forceAspectRatio = false;
            if (cssAspectRatio - maximumAspectRatio > epsilon) {
                desiredAspectRatio = maximumAspectRatio;
                forceAspectRatio = true;
            } else if (miminumAspectRatio - cssAspectRatio > epsilon) {
                desiredAspectRatio = miminumAspectRatio;
                forceAspectRatio = true;
            }
        }

        if (canvasWindowedScaleMode === 2 /* ASPECT */ || forceAspectRatio) {
            if (cssAspectRatio - desiredAspectRatio > epsilon) {
                cssWidth = cssHeight * desiredAspectRatio;
            } else if (desiredAspectRatio - cssAspectRatio > epsilon) {
                cssHeight = cssWidth / desiredAspectRatio;
            }
        }

        // Convert unconstrained render target size from CSS to physical pixel units.
        newRenderTargetWidth = canvasWindowedUseHighDpi ? (cssWidth * window.devicePixelRatio) : cssWidth;
        newRenderTargetHeight = canvasWindowedUseHighDpi ? (cssHeight * window.devicePixelRatio) : cssHeight;
    ...
    mainArea.style.marginTop = (window.innerHeight - cssHeight) / 2 + 'px';
...
// One of 0=Default, 1=Nearest, 2=Bi-linear
Module['UE4_fullscreenFilteringMode'] = 0;

// Specifies whether to use the browser's default fullscreen mode, or to use a custom fullscreen mode.
Module['UE4_useSoftFullscreenMode'] = false;

// Time to wait until pointer lock request can be triggered again after it has been cancelled.
Module['UE4_pointerLockRequestCooldown'] = 1500;


function sendMessageToUnreal(msg) {
    if (window.ue && window.ue.interface && window.ue.interface.OnMessageReceived) {
        window.ue.interface.OnMessageReceived(msg);
    } else {
        console.log("UE4 interface not found, message:", msg);
    }
}
setTimeout(() => sendMessageToUnreal("Hello from JavaScript!"), 10000);
...
//	}
	...
	function withIndexedDB(db) {
		...
		var mainCompiledCode = fetchFromIndexedDB(db, 'wasmModule').then(function(wasmModule) {
			...
			return download(Module.locateFile('ExpoTown_WebGL-HTML5-Shipping.wasm'), 'arraybuffer').then(function(wasmBytes) {
		...
		// ----------------------------------------
		// MAIN JS
		var mainJsDownload = fetchOrDownloadAndStore(db, Module.locateFile('ExpoTown_WebGL-HTML5-Shipping.js'), 'blob').then(function(data) {
		...
		// ----------------------------------------
		// MORE JS
		var dataJsDownload = fetchOrDownloadAndStore(db, Module.locateFile('ExpoTown_WebGL-HTML5-Shipping.data.js'));
...
/* // The following code would download and store the .data file as a Blob, which should be more efficient than loading an ArrayBuffer. However that seems to be buggy, so avoid it for now.
			fetchOrDownloadAndStore(db, Module.locateFile('ExpoTown_WebGL-HTML5-Shipping.data')).then(function(dataBlob) {
				return readBlobToArrayBuffer(dataBlob).then(function(dataArrayBuffer) {
					Module['preloadedPackages'] = {};
					Module['preloadedPackages'][Module.locateFile('ExpoTown_WebGL-HTML5-Shipping.data')] = dataArrayBuffer;
...
// Instead as a fallback, download as ArrayBuffer. (TODO: Figure out the bugs with the above, and switch to using that one instead)
			fetchOrDownloadAndStore(db, Module.locateFile('ExpoTown_WebGL-HTML5-Shipping.data'), 'arraybuffer').then(function(dataArrayBuffer) {
				Module['preloadedPackages'] = {};
				Module['preloadedPackages'][Module.locateFile('ExpoTown_WebGL-HTML5-Shipping.data')] = dataArrayBuffer;
				return dataJsDownload.then(addScriptToDom);
			});
